...
def check_loggedIn_and_redirect(context: BaseHTTPRequestHandler):
    ...
    if session_id and check_loggedin_by_sessionid(session_id):
        ...
        context.send_header('Location', f"http://localhost:8080/authorize?{query_string}")
        context.end_headers()
    else:
        context.send_response(302)
        context.send_header('Location', f"http://localhost:8080/authorize/login?{query_string}")
        context.end_headers()
